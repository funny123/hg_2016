...
class MemberController extends Controller
{
    ...
    }if(in_array("64",$a))
    {
        ...
        $lists = $m->where($where)->field($fields)->limit($limit)->order($order)->select();//dump($lists);die;
    ...
    public function edit_dj(){
        ...
        $where['editdecide']=1;
        list($lists, $page) = $model->getLists($where);//dump($model);die;
        $this->assign('lists', $lists);
...
//综合数据统计
    public function member_zh()
    {
        ...
        $s = M('member');
        $date=$s->order('department_x')->select();//小组称
...
//        echo $str;exit;
    ...
    public function edit_mul_do($user_id){
        ...
        $data['time_x']=strtotime(I('time_x'))?strtotime(I('time_x')):time();
        $data['editdecide']=1;
        if ($user_id==null){
            $this->error('请选择修改项！');
        }
...
// 文件上传
    ...
    public function member_edit()
    {
        ...
        if ($_POST) {
            ...
            if($result){
                $name=D('member')->where('id='.$id)->getField('realname');
    ...
    public function out_excel(){
        ...
        if(IS_POST){
            ...
            $field='id,loginname,moniname,time,realname,codeid,tel,address,agentname_x,groupname_x,department_x,birthday,jihuotime,jihuomoney,remark';
            $data=$m->field($field)->where($where)->select();
            foreach($data as $k => $v){
            ...
            D('Member')->exportData( $data,$title);
        }
        if(I('user_id')!=''){
    ...
    public function add_excel_ok1() {
        ...
        foreach($indata as $k => $v){
            ...
            if($k==0||$k==1) continue;
            if($v[6]!=''){
                if(!isMobile(trim($v[6]))){
                    $errorlist[]=$v;
                    continue;
                }
            }
            if(!isPersonalCard(trim($v[5]))){
                $errorlist[]=$v;
                continue;
            }
            $where['realname']=$v[4];
            ...
            $data=[
                ...
                'birthday'=>$v[11]?$v[11]:get_birthday($v[5]),
                'jihuotime'=>strtotime($v[12]),
                'jihuomoney'=>$v[13],
                // 'loginuser'=>$v[16],
                'remark'=>$v[14],
                //'kaihuform'=>$v[18],
    ...
    public function exporteError(){
        ...
        D('Member')->exportData(I('id')?S('errorlist'):S('repeatlist'),$title);
    }
