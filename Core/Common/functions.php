...
//生日
function date_birth($date){
    return date("m/d",($date-25569)*24*3600);
}
...
function get_birthday($id){
    return substr($id,10,2).'-'.substr($id,12,2);
}
/**
 * @param $id 开户类型
 * @return string
...
function opentype($id){
    if(substr($id,0,6)=='226001'){
        $return='网开';
    }
    if(substr($id,0,9)=='226999999'){
        $return='软件';
    }
    if(substr($id,0,8)=='22688888'){
        $return='官网';
    }
    return $return;
}
 ...
 */
 function isPersonalCard($num) {
    ...
    return preg_match('/^(\d{18,18}|\d{15,15}|\d{17,17}x|\d{17,17}X)$/', $num) ? true : false;
}